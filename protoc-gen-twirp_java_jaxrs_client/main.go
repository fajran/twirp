...
func (g *generator) Generate(in *plugin.CodeGeneratorRequest) *plugin.CodeGeneratorResponse {
	...
	for _, f := range genFiles {
		for _, service := range f.Service {
			...
			respFile := g.generateClientFile(f, service)
			if respFile != nil {
...
func (g *generator) generateClientFile(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) *plugin.CodeGeneratorResponse_File {
	g.P(`// Code generated by protoc-gen-twirp_java_jaxrs_client `, gen.Version, `, DO NOT EDIT.`)
	g.P(`// source: `, file.GetName())
	g.P()
	g.P(`package `, file.GetPackage(), `;`)
	g.P()
	...
	className := javaClassName(file, service)
	...
	for _, method := range service.Method {
		inputType := getType(file, method.GetInputType())
		outputType := getType(file, method.GetOutputType())
	...
	resp.Name = proto.String(javaFileName(file, service))
...
func javaClassName(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) string {
	return fmt.Sprintf("%s_%sClient", getProtoClass(file), serviceName(service))
}

func javaFileName(file *descriptor.FileDescriptorProto, service *descriptor.ServiceDescriptorProto) string {
	dir := strings.Replace(file.GetPackage(), ".", "/", -1)
	fname := javaClassName(file, service)
	return fmt.Sprintf("%s/%s.java", dir, fname)
}

func getType(file *descriptor.FileDescriptorProto, typeName string) string {
	return fmt.Sprintf("%s.%s", getProtoClass(file), stripPackage(typeName))
}
